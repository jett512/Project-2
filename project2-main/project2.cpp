...
/*
 ...
 algorithm FilterOdd
	...
	oddHelperfunction
		if list empty
			return odd
		else if (list1St mod2 equals 0)
			return FilterOdd(rest of list, odd)
		else if list rest empty
			return makelist(list1st(list), odd)

		odd = makelist(list1st(list), odd)
		return Helperfunction(list1st, odd)

	oddFunction	
		odd = empty list
        	return ReverseHelper(Reversed list, odd)
...
Check if list empty, if so return the odd list as the base case. 
If list is empty there is nothing to do to it.

Else if Statement checks if the first elem mod2 equals 0.
...
Finally, the recursive call is made with parameters ListRest to break down list, and the odd list

Then we move to the main odd function. 
define the list odd list and sets it to empty.
The helper function is called with parameters list reversed and odd list, and will return the final product.
The original list is reversed because when the stack is broken down the answer is reversed
so this is a correction. 
	
*/
RecursiveList FilterOddHelper(RecursiveList list, RecursiveList odd) {
	
	if (ListIsEmpty(list) == true)//base case
	{
		return odd;//nothing in the list
	} 
	else if( ListFirst(list) %2 == 0)//check if even
	{
		return FilterOddHelper(ListRest(list),odd);//return rest of list
	}
	else if(ListIsEmpty(ListRest(list)) == true)//current list first is the final elem in the list.
	{
		return MakeList(ListFirst(list), odd);//final elem of list at front followed by reversed list.
	}
	
	odd = MakeList(ListFirst(list), odd);//final elem of list at front followed by reversed list.	
	return FilterOddHelper(ListRest(list), odd);//Recursive call to break down list
}

RecursiveList FilterOdd(RecursiveList list) {
  // Implement this function.
	
	RecursiveList odd = MakeList();//defines the list reverse list and sets it to empty.
        return FilterOddHelper(Reverse(list), odd);//helper called with extra param, generates final solution
}
...
/*
 ...
 algorithm FilterEven
	...
	evenHelperfunction
		if list empty
			return even
		else if (list1St mod2 equals 1)
			return FilterOdd(rest of list, even)
		else if list rest empty
			return makelist(list1st(list), even)

		even = makelist(list1st(list), even)
		return Helperfunction(list1st, even)

	evenFunction	
		even = empty list
        	return ReverseHelper(Reversed list, even)
...
Check if list empty, if so return the evem list as the base case. 
If list is empty there is nothing to do to it.

Else if Statement checks if the first elem mod2 equals 1.
...
Finally, the recursive call is made with parameters ListRest to break down list, and the even list

Then we move to the main even function. 
define the list even list and sets it to empty.
The helper function is called with parameters list reversed and even list, and will return the final product.
The original list is reversed because when the stack is broken down the answer is reversed, so this is a correction. 

*/

RecursiveList FilterEvenHelper(RecursiveList list, RecursiveList even) {
	
	if (ListIsEmpty(list) == true)//base case
	{
		return even;//nothing in the list
	} 
	else if( ListFirst(list) %2 == 1)//check if odd
	{
		return FilterEvenHelper(ListRest(list), even);//return rest of list
	}
	else if(ListIsEmpty(ListRest(list)) == true)
	{
		return MakeList(ListFirst(list), even);//update list
	}
	//must be even
	
	even = MakeList(ListFirst(list), even);//update list
		
	return FilterEvenHelper(ListRest(list), even);//Recursive call to break down list
}

RecursiveList FilterEven(RecursiveList list) {
  // Implement this function.
	
	RecursiveList even = MakeList();//defines the list reverse list and sets it to empty.
        return FilterEvenHelper(Reverse(list), even);//helper called with extra param, generates final solution
}
...
/*
 ...
 algorithm Append
	...
	Append Function
		pass reverse first_list and second list to helper function
	
 COMMENTS:
Helperfunction is used to have first_list reversed in the main function.
Without doing this, the first list would be reverse in the final output.
...
main function passes the parameters to the helper function, the the first list is reversed to produce the correct result. 
...
RecursiveList AppendHelper(RecursiveList first_list, RecursiveList second_list){
 if( ListIsEmpty(first_list) == true)//Check if list empty
	...
	else if(ListIsEmpty(ListRest(first_list)) == true)//current list first is the final elem in the first_list.
	{
		...
		return AppendHelper(ListRest(first_list), MakeList(n, second_list));//recurisve call to break down list
}
RecursiveList Append(RecursiveList first_list, RecursiveList second_list) {
  // Implement this function.
	
	
	return AppendHelper(Reverse(first_list), second_list);//pass reverse first_list and second list to helper
